...
class SwarmManager:
    def __init__(self):
        ...
        try:
            ...
            print(f"[🛡️ SEC] Swarm Manager successfully blocklisted rogue node: {attacker_ip}")
        except Exception:
            pass
    ...
    def heartbeat_loop(self, model):
        while True:
            ...
            try:
                ...
                # 🔥 THE FIX: If the leader's API is dead, blacklist their IP!
                if self.current_leader_ip != "leader" and self.current_leader_ip != self.my_ip:
                    print(f"[🚨 SWARM] Leader {self.current_leader_ip} is a Zombie (Ping works, API dead). Blacklisting!")
